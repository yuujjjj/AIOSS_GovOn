# M1: Planning (Week 1-4)

## 산출물 체크리스트

### Week 1: 프로젝트 킥오프
- [ ] kickoff_meeting_notes.md - 킥오프 회의록
- [ ] requirements_spec.md - 요구사항 명세서
- [ ] tech_stack_report.md - 기술 스택 선정 보고서
- [ ] exaone_analysis.md - EXAONE-Deep-7.8B 모델 분석

### Week 2: 시스템 설계
- [x] architecture_diagram.png - 시스템 아키텍처
- [x] database_erd.png - ERD
- [x] api_specification.md - API 명세서
- [ ] vllm_serving_design.md - vLLM 서빙 구조

### Week 3: 개발 환경
- [ ] dev_environment_guide.md - 개발 환경 가이드
- [ ] git_workflow.md - Git 워크플로우
- [ ] crawler_prototype/ - 크롤러 초안

### Week 4: 데이터 수집
- [ ] crawling_targets.md - 크롤링 대상 목록
- [ ] preprocessing_script.py - 전처리 스크립트
- [ ] data_quality_report.md - 데이터 품질 리포트

## 완료 기준
- [ ] 10,000건+ 민원 데이터 수집
- [ ] 개인정보 비식별화 완료
- [ ] 아키텍처 문서 승인

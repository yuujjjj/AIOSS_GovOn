...
## M2 미달성 KPI 분석 및 M3 고도화 연계

### 미달성 KPI 상세 분석

| 지표 | M2 측정값 | 목표 | 갭 | 원인 분석 | M3 고도화 전략 |
|------|-----------|------|-----|-----------|---------------|
| BLEU | 17.32 | >= 30 | -12.68 (42.3% 미달) | 참조-생성 스타일 불일치, 1 epoch 학습 한계, 반복 생성 | #67 QLoRA HP 최적화, #68 답변 품질 고도화 |
| ROUGE-L | 18.28 | >= 40 | -21.72 (54.3% 미달) | 참조 답변(짧은 요약) vs 생성(공식 답변), PII 마스킹 영향 | #68 답변 품질 고도화, #70 데이터 증강 |
| 추론 속도 (Avg) | 9.29s | < 2s | -7.29s | HuggingFace generate() 직접 호출, vLLM 미적용 | #69 추론 속도 최적화 (M3에서 2.43s까지 개선) |
| 분류 정확도 | 0% (2%) | >= 85% | -85%p | thought 태그 파싱 실패, 테스트셋 편향(other만) | #52 프롬프트/파서 최적화 (M3에서 90% 달성) |

### 미실행 실험

M2 실험 계획서에 명시되었으나 미실행된 실험:
- **EXP-002 (Rank 변화)**: r=8, r=32 실험 미실행 -> #67에서 수행 예정
- **EXP-003 (Learning Rate 변화)**: lr=1e-4 실험 미실행 -> #67에서 수행 예정
- **Multi-epoch 학습**: 3 epoch 전체 학습 미실행 (1 epoch만 수행) -> #67에서 수행 예정

### 핵심 발견사항

1. **데이터 품질 문제**: 테스트 세트가 `other`(금융 서비스) 카테고리에 편중되어 분류 정확도가 비정상적으로 측정됨
2. **평가 방법론 한계**: BLEU/ROUGE는 스타일 불일치에 민감하여 실제 답변 품질을 정확히 반영하지 못함
3. **반복 생성 패턴**: repetition_penalty 미적용 상태에서 동일 문구 반복 생성 관찰
4. **양자화 성공**: 모델 크기(4.94GB)와 VRAM(4.95GB) 목표는 충분히 달성

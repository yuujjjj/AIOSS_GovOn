...
- `Bootstrap`, `Bulma`는 완성형 컴포넌트/클래스 기반 접근에 가깝고, `Tailwind CSS`는 utility-first 접근입니다.
...
- UI 라이브러리 일부와 혼합 사용이 가능하고 유연성이 높음
- 다만 현재 GovOn의 UI 라이브러리 선택 결과와 가장 자연스럽게 연결되는 방향은 아님

...
product completeness. The Cloud Run service is deployed as a demo container with
`SKIP_MODEL_LOAD=true`, so the success criteria are container startup, `/health`
availability, revision visibility, and monitoring signal collection.
...
- `Deploy Container to Cloud Run` shows `Deploy to Cloud Run` and `Health Check` as successful, not skipped.
- The Cloud Run deploy summary includes service name, region, image, URL, and revision.
- The Cloud Run health check summary shows `/health` with HTTP 200.
- `Cloud Run Monitoring` shows service URL, status, HTTP code, latency, revision, and timestamp.

## Responsibility Split

Codex can maintain the workflow files, Docker strategy documentation, frontend
static export configuration, and local build validation.
...
- GCP secrets must be configured: `GCP_SA_KEY`, `GCP_PROJECT_ID`.
- GCP variables should be configured or defaults accepted: `GCP_REGION`, `GCP_AR_REPOSITORY`, `GCP_SERVICE_NAME`.

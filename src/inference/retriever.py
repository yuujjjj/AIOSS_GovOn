...
class CivilComplaintRetriever:
    ...
    def build_index(self, data_path: str, save_path: Optional[str] = None):
            ...
            for line in f:
                try:
                    ...
                    if "[|assistant|]" in item['text']:
                        answer = item['text'].split("[|assistant|]")[1].split("[|endofturn|]")[0].strip()
        ...
        embeddings = self.model.encode(complaints, show_progress_bar=True, normalize_embeddings=True)
        
        dimension = embeddings.shape[1]
        self.index = faiss.IndexFlatIP(dimension) # Inner Product for cosine similarity with normalized embeddings

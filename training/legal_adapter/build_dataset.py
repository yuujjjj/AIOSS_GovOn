...
# 설정
# ---------------------------------------------------------------------------
PROJECT_ROOT = Path(os.environ.get("GOVON_ROOT", Path(__file__).resolve().parents[2]))
...
def load_hf_precedents() -> list[dict]:
    ...
    for row in tqdm(ds, desc="HF 판례"):
        ...
        if _is_valid(rec):
            records.append(rec)
    print(
        f"  HF 판례: {len(ds):,}건 -> 참조조문+판시사항 존재 {total:,}건 -> 필터 후 {len(records):,}건"
    )
    return records
...
def _parse_taskinfo_source(
    ...
    records: list[dict] = []
    total = 0
    tl_dirs = sorted(p for p in base_dir.iterdir() if p.is_dir() and p.name.startswith("TL_"))
